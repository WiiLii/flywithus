...
function generatePageTitle( $categoryName = "Singapore Polytechnic Carbon Footprint Challenge" )
{
    echo '<script>
...
function generateFeedbackForm( $spiceID )
{
    $name = getUsername();
    global $connection;

    $query = "select question, questionID, inputType from awaretbl";
    ...
    while ( $row = mysqli_fetch_array( $result ) ) {
        ...
        generateFeedbackQnA( $row[ 1 ], $row[ 2 ], $questionIndex );
    ...
    echo '</div>
          ...
          <p>Thanks,<span class="name"> '.$name.'!</span> Your action to reduce carbon footprint will help save the world!</p>
          </div>
...
function generateFeedbackQnA( $questionID, $inputType, $questionIndex )
{

    global $connection;

    $query = "SELECT answer, answerID FROM awareanstbl, awaretbl"
           . " WHERE (awareanstbl.questionID = awaretbl.questionID)"
           . " AND awaretbl.questionID = ?";
        ...
        if ( $result ) {
            ...
            if ( $inputType == null || $inputType == "r" ) {
                echo '<div class="funkyradio-primary">
                      <input id="radio' .$answerID. '" type="radio" name="Aware' . $questionIndex . '" required value="' . $answer . '"/>
...
function generatePrevNextButton( $countIndex, $endIndex )
{
    ...
    if ( $countIndex == 0 ) {
        ...
        echo '<form action="CommonSurvey.php" id="prevSurveyForm" style="display: inline;" method="POST">';
...
{   $name = getUsername();
    echo '<div id="falseBack">
          ...
          <li>
          <form id="commonSurveyForm" action="CommonSurvey.php" method="POST">
          <input type="submit" value="Common" class="btn btn-default navbar-btn" style="background: transparent; border: none; color: #D10052; font-weight: bold;" />
          </form>
          </li>
    ';
